...
ntp_whitelist = ['77.236.182.128', '8.120.162.35', '91.2 10.88.37', '162.159.200.123', '120.25.112.107']
...
#def db_query(time_ago_var, time_now_var):
def db_query():
    result = client.query('SELECT received as "time", in_bytes, proto, src_port, src4_addr FROM siem.netflow WHERE (time >= \'2025-07-04 12:22:29\' AND time <= \'2025-07-04 12:24:50\') AND (proto=\'udp\') AND (src_port=123)')
    ...
    sum_query_test = client.query(f'SELECT sum(in_bytes) from siem.netflow WHERE (received >= \'2025-07-04 12:22:29\' AND received <= \'2025-07-04 12:24:50\') AND (proto=\'udp\') AND (src_port=123)')
...
malicious_ntp_ips = db_query_result[0]  # return IPs
ntp_bytes_received = db_query_result[1]
count_ntp_hosts = db_query_result[2]



#def ntp_event_gen(time_now_var, ntp_bytes_received, count_ntp_hosts):
def ntp_event_gen():
    ...
    if ntp_bytes_received > bytes_threshold:
        ...
        import configurator
        #junos_config(malicious_ntp_ips, attack_key)
        napalm_junos_config(malicious_ntp_ips, attack_key)

        print(count_ntp_hosts)
